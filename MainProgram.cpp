...
MyString MyString::toUpperCase() const {
    ...
    string upStr = data;
    transform(upStr.begin(), upStr.end(), upStr.begin(), [](unsigned char c) {return toupper(c);});
    return MyString(upStr);
...
MyString MyString::toLowerCase() const {
    ...
    // Do NOT modify the original object
    string lowStr = data;
    transform(lowStr.begin(), lowStr.end(), lowStr.begin(), [](unsigned char c) {return tolower(c);});
    return MyString(lowStr);
}
